...
<div id="wrapper">
  ...
  <nav class="navbar navbar-default navbar-static-top" role="navigation"
	...
	<div class="navbar-header">
		<a class="navbar-brand" href="<%=basePath%>customer/list">BOOT客户管理系统</a>
	</div>
	<!-- 导航栏右侧图标部分 -->
						...
						<tbody>
							<c:forEach items="${page.rows}" var="row">
								<tr>
									<td>${row.cus_id}</td>
									<td>${row.cus_name}</td>
									<td>${row.cus_source}</td>
									<td>${row.cus_industry}</td>
									<td>${row.cus_level}</td>
									<td>${row.cus_phone}</td>
								    <td>${row.cus_mobile}</td>
									<td>
										<a href="#" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#customerEditDialog" onclick= "editCustomer(${row.cus_id})">修改</a>
										<a href="#" class="btn btn-danger btn-xs" onclick="deleteCustomer(${row.cus_id})">删除</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
			...
			<div class="modal-body">
				<form class="form-horizontal" id="new_customer_form">
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" class="form-control" id="new_customerName" placeholder="客户名称" name="cus_name" />
						</div>
					</div>
					<div class="form-group">
						<label for="new_customerFrom" style="float:left;padding:7px 15px 0 27px;">客户来源</label> 
						<div class="col-sm-10">
							<select	class="form-control" id="new_customerFrom" name="cus_source">
								<option value="">--请选择--</option>
								<c:forEach items="${fromType}" var="item">
					...
					<div class="form-group">
						...
						<div class="col-sm-10"> 
							<select	class="form-control" id="new_custIndustry"  name="cus_industry">
								<option value="">--请选择--</option>
								<c:forEach items="${industryType}" var="item">
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<select	class="form-control" id="new_custLevel" name="cus_level">
								<option value="">--请选择--</option>
								<c:forEach items="${levelType}" var="item">
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" class="form-control" id="new_linkMan" placeholder="联系人" name="cus_linkman" />
						</div>
					</div>
					<div class="form-group">
						<label for="new_phone" class="col-sm-2 control-label">固定电话</label>
						<div class="col-sm-10">
							<input type="text" class="form-control" id="new_phone" placeholder="固定电话" name="cus_phone" />
						</div>
					</div>
					<div class="form-group">
						<label for="new_mobile" class="col-sm-2 control-label">移动电话</label>
						<div class="col-sm-10">
							<input type="text" class="form-control" id="new_mobile" placeholder="移动电话" name="cus_mobile" />
						</div>
					</div>
					<div class="form-group">
						<label for="new_zipcode" class="col-sm-2 control-label">邮政编码</label>
						<div class="col-sm-10">
							<input type="text" class="form-control" id="new_zipcode" placeholder="邮政编码" name="cus_zipcode" />
						</div>
					</div>
					<div class="form-group">
						<label for="new_address" class="col-sm-2 control-label">联系地址</label>
						<div class="col-sm-10">
							<input type="text" class="form-control" id="new_address" placeholder="联系地址" name="cus_address" />
						</div>
					</div>
				</form>
			</div>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form class="form-horizontal" id="edit_customer_form">
					<input type="hidden" id="edit_cust_id" name="cus_id"/>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<select	class="form-control" id="edit_customerFrom" name="cus_source">
								<option value="">--请选择--</option>
								<c:forEach items="${fromType}" var="item">
					...
					<div class="form-group">
						...
						<div class="col-sm-10"> 
							<select	class="form-control" id="edit_custIndustry"  name="cus_industry">
								<option value="">--请选择--</option>
								<c:forEach items="${industryType}" var="item">
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<select	class="form-control" id="edit_custLevel" name="cus_level">
								<option value="">--请选择--</option>
								<c:forEach items="${levelType}" var="item">
									<option value="${item.dict_id}"<c:if test="${item.dict_id == custLevel}"> selected</c:if>>${item.dict_item_name }</option>
								</c:forEach>
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" class="form-control" id="edit_linkMan" placeholder="联系人" name="cus_linkman" />
						</div>
					</div>
					<div class="form-group">
						<label for="edit_phone" class="col-sm-2 control-label">固定电话</label>
						<div class="col-sm-10">
							<input type="text" class="form-control" id="edit_phone" placeholder="固定电话" name="cus_phone" />
						</div>
					</div>
					<div class="form-group">
						<label for="edit_mobile" class="col-sm-2 control-label">移动电话</label>
						<div class="col-sm-10">
							<input type="text" class="form-control" id="edit_mobile" placeholder="移动电话" name="cus_mobile" />
						</div>
					</div>
					<div class="form-group">
						<label for="edit_zipcode" class="col-sm-2 control-label">邮政编码</label>
						<div class="col-sm-10">
							<input type="text" class="form-control" id="edit_zipcode" placeholder="邮政编码" name="cus_zipcode" />
						</div>
					</div>
					<div class="form-group">
...
//创建客户
	...
	function editCustomer(id){
		$.ajax({
			...
			success:function(data){
				$("#edit_cust_id").val(data.cus_id);
	            $("#edit_customerName").val(data.cus_name);
	            $("#edit_customerFrom").val(data.cus_source)
	            $("#edit_custIndustry").val(data.cus_industry)
	            $("#edit_custLevel").val(data.cus_level)
	            $("#edit_linkMan").val(data.cus_linkman);
	            $("#edit_phone").val(data.cus_phone);
	            $("#edit_mobile").val(data.cus_mobile);
	            $("#edit_zipcode").val(data.cus_zipcode);
	...
	function updateCustomer(){
		$.post("<%=basePath%>customer/update",$("#edit_customer_form").serialize(),
		function(data){
			if(data =="OK"){
				alert("客户信息更新成功！");
				window.location.reload();
	...
	function deleteCustomer(id){
		if(confirm('确定要删除该客户吗?')){
			$.post("<%=basePath%>customer/deletes",{"id":id},
			function(data){
				if(data =="OK"){
				alert("客户删除成功！");
				window.location.reload();
